...
@section('content')
    <div class="container-fluid">
        ...
        <div class="alert alert-info">
            ...
            <p>{{ __('voyager::settings.usage_help') }} <code>settings('group.key')</code></p>
        </div>
        @endif
    </div>
                ...
                <div class="page-content settings container-fluid">
                    ...
                    <div class="tab-content">
                        ...
                        <div id="{{ \Illuminate\Support\Str::slug($group) }}" class="tab-pane fade in @if($group == $active) active @endif">
                            ...
                            <div class="setting_key_{{ str_replace(['-', '.'], '_', $setting->key) }}"
                                @if (is_translatable_setting($setting->key, true) && translatable_setting_keylang($setting->key) !== config('voyager.multilingual.default')) style="display:none" @endif
                            >
                            <div class="panel-heading">
                                <h3 class="panel-title">
                                    {{ $setting->display_name }}
                                    @if (is_translatable_setting($setting->key, true))
                                        @if(config('voyager.show_dev_tips'))<code>settings('{{ translatable_setting_key($setting->key) }}')</code>@endif
                                        <div class="translatable-setting-switch btn-group btn-group-sm" role="group" >
                                            @foreach (config('voyager.multilingual.locales') as $lang)
                                                <label class="btn btn-primary{{ ($lang === translatable_setting_keylang($setting->key)) ? " active" : "" }}"
                                                    data-lang="{{$lang}}" data-key="{{str_replace(['-', '.'], '_', translatable_setting_key($setting->key))}}">
                                                    {{ strtoupper($lang) }}
                                                </label>
                                            @endforeach
                                        </div>
                                    @else
                                        @if(config('voyager.show_dev_tips'))<code>settings('{{ $setting->key }}')</code>@endif
                                    @endif
                                </h3>
...
@section('javascript')
    <script>
        var voyager_multilingual_locales = @json(config('voyager.multilingual.locales'));
        $('document').ready(function () {
            $('.translatable-setting-switch .btn').click(function (event) {
                var $this = $(this);
                var key = $this.attr('data-key');
                $('.setting_key_' + key).hide();
                $.each(voyager_multilingual_locales, function(index, lang) {
                    $('.setting_key_' + key + '_' + lang).hide();
                });
                $('.setting_key_' + key + '_' + $this.attr('data-lang')).show();
            });
        });
    </script>
    <script>
